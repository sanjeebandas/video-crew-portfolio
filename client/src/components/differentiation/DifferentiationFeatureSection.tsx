...
const DifferentiationFeatureSection = ({
  ...
  useEffect(() => {
    // Individual animation for each feature section
    const timer = setTimeout(() => {
      // Each section animates individually when it comes into view
      stackIn(".diff-feature-section", {
        duration: 0.6,
        ease: "power1.out",
        scrollTrigger: {
          trigger: ".diff-feature-section",
          start: "top 80%",
          end: "bottom 20%",
  ...
  return (
    <section className="w-full bg-black text-white py-12">
      <div
        className={`diff-content-rail max-w-[1248px] mx-auto px-4 sm:px-6 md:px-6 flex flex-col md:flex-row items-center ${
      ...
      >
        ...
        <div className="w-full md:w-1/2 flex flex-col items-center md:items-start text-center md:text-left relative group">
          ...
          <h3 className="text-2xl md:text-3xl font-bold mb-2 group-hover:text-blue-400 transition-colors duration-300 ease-out">
            {title}
          </h3>
