      ...
      <div className="max-w-[1248px] mx-auto">
        ...
        <div className="hidden min-[1024px]:grid grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-2 gap-6 col-span-2">
            {workCultureItems.slice(0, 2).map((item, idx) => (
              <div
                key={idx}
                className="work-culture-card relative w-[613px] h-[548px] overflow-hidden shadow-md border border-white/10 transition-transform duration-300 ease-in-out hover:-translate-y-3"
              >
                <img
                  src={item.image}
                  alt={item.title}
                ...
                />
                 ...
                 <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                   <div className="flex flex-col gap-2">
                     <p className="work-culture-text text-xs text-gray-300 uppercase tracking-wide">
                       {item.subtitle}
                     </p>
                     <h3 className="work-culture-text text-lg font-bold mb-2">{item.title}</h3>
                     <p className="work-culture-text text-sm text-gray-200 leading-relaxed">
                       {item.description}
                     </p>
                   </div>
                 </div>
          ...
          <div className="col-span-2">
            <div className="work-culture-card relative w-[1248px] h-[560px] overflow-hidden shadow-md border border-white/10 transition-transform duration-300 ease-in-out hover:-translate-y-3">
              <img
              ...
              />
               ...
               <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                 <div className="flex flex-col gap-2">
                   ...
                   <h3 className="work-culture-text text-xl font-bold mb-2">
                     {workCultureItems[2].title}
                   </h3>
                   <p className="work-culture-text text-base text-gray-200 leading-relaxed max-w-2xl">
        ...
        <div className="min-[1024px]:hidden grid grid-cols-1 gap-6 place-items-center">
          {workCultureItems.map((item, idx) => (
            <div
              key={idx}
              className="work-culture-mobile-card relative w-[302px] h-[257px] min-[768px]:w-[360px] min-[768px]:h-[300px] rounded-lg overflow-hidden shadow-md border border-white/10 transition-transform duration-300 ease-in-out hover:-translate-y-3"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              />
               ...
               <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                 <div className="flex flex-col gap-1">
                   <p className="work-culture-mobile-text text-xs text-gray-300 uppercase tracking-wide">
                     {item.subtitle}
                   </p>
                   <h3 className="work-culture-mobile-text text-base font-bold mb-1">{item.title}</h3>
                   <p className="work-culture-mobile-text text-xs text-gray-200 leading-snug">
                     {item.description}
                   </p>
                 </div>
               </div>
