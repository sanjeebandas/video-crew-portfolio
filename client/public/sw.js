...
const API_CACHE = ["/api/portfolio", "/api/contact", "/api/notifications"];
...
async function handleNavigation(request) {
  try {
    ...
    console.log(
      "Service Worker: Navigation failed, serving offline page",
      error
    );
    ...
    for (const request of pendingRequests) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            "Service Worker: Background sync successful for",
            request.url
          );
        }
      } catch (error) {
        console.error(
          "Service Worker: Background sync failed for",
          request.url,
          error
        );
      }
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification("비디오크루", options));
});
